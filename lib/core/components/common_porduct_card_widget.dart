...
class CommonProductCardWidget extends StatefulWidget {
  const CommonProductCardWidget({super.key, this.imageUrl}); // Added imageUrl
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      ...
      child: ClipRRect(
        ...
        borderRadius:
            BorderRadius.circular(11).r, // Assuming 11.r for consistency
        child: (widget.imageUrl != null && widget.imageUrl!.isNotEmpty)
            ? CustomNetworkImage(
                imageUrl: widget.imageUrl,
                memCache: CustomMemCache(
                  width: (context.mediaQuery.size.width / 2 - 20)
                      .toInt()
                      .withDevicePixel(
                          context), // Adjust width based on typical card size
                ),
              )
            : Container(
